...
def register_family_lora_args(parser):
    ...
    for path in family_handlers:
        # 部分家族（例如依赖 torchaudio 的 chatterbox）在当前环境下可能因底层库不兼容而导入失败。
        # 为了避免整个程序在导入阶段崩溃，这里对单个 family 的导入做保护性处理：
        try:
            handler = importlib.import_module(path).family_handler
        except Exception as e:
            # 仅在命令行初始化阶段打印警告，继续处理其他家族
            print(f"[wgp] Warning: failed to import family handler '{path}', skip registering its LoRA args. Error: {e}")
            continue
...
def _parse_args():
    ...
    # 使用 parse_known_args 避免在存在未知参数（例如 RunPod 注入的参数）时直接退出（exit code 2）
